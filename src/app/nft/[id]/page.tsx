...
export default function NftPage() {
  ...
  const rpcUrl = useMemo(
    () =>
      tokenId && nft?.symbol
        ...
        : null,
    [config.nexus, config.rpcBaseUrl, nft, tokenId],
  );
  ...
  const isCrownNft = isCrownSymbol(nft?.symbol);
  const mediaVideoUrl =
    !nft ? null : isCrownNft ? CROWN_FULL_VIDEO_URL : (nft.nft_metadata?.videoURL ?? null);
  const mediaImageUrl = (() => {
    if (!nft) return null;
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {attributes.map((attribute) => (
                  ...
                  >
                    ...
                    <div className="mt-1 text-foreground">{attribute.render}</div>
                  </div>
                ))}
              </div>
            </div>
